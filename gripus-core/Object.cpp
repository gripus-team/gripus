...
Object::Object()
	: id(""), mesh(), mass(0.0f), charge(0.0f), position(0.0, 0.0, 0.0), centerOfMass(0.0, 0.0, 0.0), velocity(0.0f, 0.0f, 0.0f), orientation(0.0f, 0.0f, 0.0f), rotation(0.0f, 0.0f, 0.0f) {
		
}
Object::Object(std::string id, std::string mesh, float mass, float charge, glm::vec3 position, glm::vec3 centerOfMass, glm::vec3 velocity, glm::vec3 orientation, glm::vec3 rotation)
	: id(id), mesh(mesh), mass(mass), charge(charge), position(position), centerOfMass(centerOfMass), velocity(velocity), orientation(orientation), rotation(rotation) {
		
}
Object::Object(Object* object)
	: id(object->id), mesh(object->mesh), mass(object->mass), charge(object->charge), position(object->position), centerOfMass(object->centerOfMass), velocity(object->velocity), orientation(object->orientation), rotation(object->rotation) {
...
sdph(Object) {
	...
	myElement->SetAttribute("charge", std::to_string(this->charge).c_str());
...
dsdph(Object) {
	...
	a = myElement->Attribute("charge");
	if (a)
		this->charge = std::stof(a);
